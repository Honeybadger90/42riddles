...
int main() { 
  ...
  /* seek second record */
  fseek(fptr, 1*sizeof(item), SEEK_SET); /* setting the size of from zero to one enables the second part to print the 2nd id with SEEK_END  */
  fread(&secondf, 1, sizeof(item), fptr);
  printf("%d  %s\n", secondf.id, secondf.name);

  fseek(fptr, 0*sizeof(item), SEEK_END);   /* setting here SEEK_SET to SEEK_END returns 2nd id */
  fread(&secondf, 1, sizeof(item), fptr);
  printf("%d  %s\n", secondf.id, secondf.name);
